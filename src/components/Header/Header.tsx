        ...
        <Tooltip title="Logar">
          <IconButton onClick={openMenu} sx={{ p: 0, color: 'white', height: '100%', width: '100%' }}>
            <StyledBadge
              ...
              online={!!user}
            >
              <Icon icon="solar:user-bold" height='28px' ref={ref} />
            </StyledBadge>
          </IconButton>
        </Tooltip>
        <Menu
          ...
          <Collapse in={layoutOpen} timeout="auto" unmountOnExit sx={{ width: '100%',
            ...
            <Box px={2} py={1}>
              ...
              sx={{ width: '100%' }}
                ...
                value={user?.layoutScale?.card || 0}
                onChange={(_, value) =>
                  setUser(prev => prev ? { 
                    ...prev, 
                    layoutScale: { 
                      card: value as number, 
                      board: prev.layoutScale?.board ?? 1 
                    }
                  } : prev)
                }
            ...
            <Box px={2} py={1}>
              ...
              sx={{ width: '100%' }}
                aria-label="layout-slider"
                value={user?.layoutScale?.board || 0}
                onChange={(_, value) =>
                  setUser(prev => prev ? { 
                    ...prev, 
                    layoutScale: { 
                      card: prev.layoutScale?.card ?? 1, 
                      board: value
                    }
                  } : prev)
                }
